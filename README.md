...
Ставится поверх стокового AntiZapret через штатный хук `custom-doall.sh`, поэтому переживает обновления. 

## Зачем

AntiZapret для «не-заблокированных» доменов резолвит их через РФ-резолверы и оставляет на прямом маршруте. Иногда гео-DNS отдаёт для домена IP из подсети, которую тот же аплинк сам же и блокирует (пример — `amp-api.apps.apple.com` → `146.75.119.x`). Этот инструмент позволяет:
